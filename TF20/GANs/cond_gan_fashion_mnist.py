...
Store images in a folder and use convert command to create a gif
...
def get_generator(latent_dimension):
    # condition subnetwork: encode the condition
    # in a hidden representation
    ...
    # concatenate the hidden condition
    # representation to noise and upsample
...
def get_discriminator():
    ...
    net = tf.keras.layers.Conv2D(
        ...
        padding="same",
        strides=2,
        activation=tf.nn.relu)(image) # 14x14x14
    net = tf.keras.layers.Conv2D(
        depth*2, kernel_size,
        padding="same",
        strides=2,
...
# Losses for Adversarial training process
...
def train():
    ...
    def train_step(image, label):
        ...
        # deleting the tape as it is defined as persistent
        del tape
    ...
    for epoch in range(epochs):
        ...
        plt.title('Epoch {}: {}'.format(epoch, info.features['label'].int2str(
            tf.squeeze(tf.cast(condition, tf.int32)).numpy())))
        plt.imshow(tf.squeeze(generated).numpy(), cmap='gray')
        plt.savefig('./image/image_{}.png'.format(epoch))
        plt.pause(0.05)
